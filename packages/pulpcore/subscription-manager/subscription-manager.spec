...
%if !(0%{?fedora} < 30 && %{with python3}) || 0%{?rhel} >= 7
...
%global use_cockpit 0%{?fedora} || 0%{?rhel} >= 8
...
Source0: https://codeload.github.com/candlepin/subscription-manager/tar.gz/%{name}-%{version}-1#/%{name}-%{version}.tar.gz
...
%if !%{use_container_plugin}
Obsoletes: subscription-manager-plugin-container > 0
%endif
...
%if %{use_yum}
    ...
    %dir %{_usr}/lib/yum-plugins/
%endif

%if %{use_dnf}
    %dir %{_sysconfdir}/dnf
    %dir %{_sysconfdir}/dnf/plugins
    %dir %{_usr}/lib/dnf-plugins/
%endif
...
%if %{use_yum}
    %{_usr}/lib/yum-plugins/subscription-manager.py*
    %{_usr}/lib/yum-plugins/product-id.py*
    %{_usr}/lib/yum-plugins/search-disabled-repos.py*
%endif

%exclude %{_usr}/lib/yum-plugins/__pycache__/

# zypper plugins
%if 0%{?suse_version}
%{_usr}/lib/zypp/plugins/services/rhsm
%endif
