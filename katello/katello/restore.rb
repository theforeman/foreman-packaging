...
module KatelloUtilities
  class Restore
    ...
    def reset_katello
      ...
      installer << if @is_foreman_proxy_content
                     " --scenario #{@foreman_proxy_content} --foreman-proxy-register-in-foreman false"
                   else
                     " --scenario #{@program}"
                   end
    ...
    def restore_databases
      ...
      if !@mongo_data_exists && !@pgsql_data_exists && !valid_logical_backup
        restore_psql_dumps if @foreman_dump_exists && @candlepin_dump_exists
        restore_mongo_dump if @mongo_dump_exists
      end
      migrate_pulp
      puts "Done.\n"
    end
    ...
    def display_backup_options
      ...
      if @is_foreman_proxy_content
        puts "---- A #{@foreman_proxy_content.tr('-', ' ')} backup directory contains: #{@foreman_proxy_content_backup_files.join(', ')}"
        puts "---- A #{@foreman_proxy_content.tr('-', ' ')} backup directory contains: #{@foreman_proxy_content_online_backup_files.join(', ')}"
      else
    ...
    def backup_valid?
      ...
      @mongo_dump_exists = Dir.exist?(File.join(@dir, 'mongo_dump'))
