...
module KatelloUtilities
  class HostnameChange
    ...
    def check_for_certs_tar
      ...
      if @options[:certs_tar]
        ...
        fail_with_message("You must specify --certs-tar argument when on a #{@proxy}." \
                               ...
                               "#{@proxy.downcase.tr(' ', '-')}-certs-generate and copied to this machine.")
      end
    end
    ...
    def run
      ...
      run_cmd("yum remove -y #{cert_rpms}")
      ...
      installer_output = run_cmd(installer.to_s)
      installer_success = $CHILD_STATUS.success?
