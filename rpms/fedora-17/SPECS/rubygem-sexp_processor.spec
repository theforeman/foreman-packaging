...
# anyway
find %{buildroot}/usr/share/gems/gems/sexp_processor-3.1.0/test/ -type f | \
  xargs -n 1 sed -i -e '/^#!\/usr\/.*\/ruby.*/d'
# Ships with extremely tight permissions, bring them inline with other gems
find %{buildroot}/usr/share/gems/gems/sexp_processor-3.1.0/ -type f | \
  xargs chmod 0644
...
%exclude %{gem_cache}
%{gem_spec}
/usr/share/gems/gems/sexp_processor-3.1.0/
%files doc
%doc %{gem_docdir}
