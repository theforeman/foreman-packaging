...
generate_gem_package() {
	...
	if [[ $UPDATE == true ]]; then
		...
		rm $CHANGELOG
		rpmdev-bumpspec -c "- Regenerate spec file based on the current template" $PACKAGE_NAME.spec
	else
...
generate_gem_package
if [[ $UPDATE == true ]] ; then
	git commit -m "Regenerate $PACKAGE_NAME based on the current template"
else
	add_to_tito_props
	add_gem_to_comps
