...
generate_npm_package() {
  ...
  npm2rpm -n $NPM_MODULE_NAME -v $VERSION -s $STRATEGY -o $PACKAGE_DIR --use-legacy-peer-deps --release "$release"
  echo "FINISHED"
  if [[ $UPDATE == true ]]; then
  ...
  echo "$CHANGELOG" | "$SCRIPT_ROOT"/add_changelog.sh $SPEC_FILE "${VERSION}-${release}"
  echo "FINISHED"
...
if [[ $UPDATE == true ]] ; then
  ...
  if [[ $VERSION == "$EXISTING_VERSION" ]] ; then
    ...
    if [[ $REWRITE_ON_SAME_VERSION == true ]] ; then
      if [[ -z $STRATEGY ]] ; then
        if grep -q registry.npmjs.org.tgz "$SPEC_FILE" ; then
          STRATEGY="bundle"
        else
          STRATEGY="single"
        fi
      fi
      release=$(rpmspec --query --queryformat "%{release}" "$SPEC_FILE")
      generate_npm_package "$(( ${release%.*} + 1 ))"
    fi
  else
    if [[ -z $STRATEGY ]] ; then
      suggest_strategy
